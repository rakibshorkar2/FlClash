...
case "${PLATFORM_NAME:-macosx}" in
  iphoneos)
    ...
    ;;
  iphonesimulator)
    # The Go core only targets devices: Network Extension is unavailable on
    # the simulator. Skip building and let Xcode link against the device lib.
    echo "buildkit: skipping iOS core build for simulator (requires a device)"
    exit 0
    ;;
  *)
    case "${ARCHS:-}" in
