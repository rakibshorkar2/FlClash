...
class _IosToolchain {
  const _IosToolchain({required this.cc, required this.sysroot});

  final String cc;
  final String sysroot;
}

_IosToolchain _resolveIosToolchain() {
  final ccResult = runCommand(
    'xcrun',
    ['--sdk', 'iphoneos', '--find', 'clang'],
  );
  final sysrootResult = runCommand(
    'xcrun',
    ['--sdk', 'iphoneos', '--show-sdk-path'],
  );
  return _IosToolchain(
    cc: (ccResult.stdout as String).trim(),
    sysroot: (sysrootResult.stdout as String).trim(),
  );
}
...
class GoBuilder {
  ...
  Future<BuildExecution> build(Target target, {bool force = false}) async {
    ...
    return cache.run(
      ...
      build: () async {
        ...
        if (target.goos == 'ios') {
          outputs.addAll(
            await _adjustIOSOutput(outDir: outDir, libPath: outFile),
          );
        }
  ...
  Map<String, String> _buildEnvironment(Target target) {
    ...
    if (target.isLib) {
      env['CGO_ENABLED'] = '1';
      if (target.goos == 'ios') {
        final toolchain = _resolveIosToolchain();
        env
          ..['CC'] = toolchain.cc
          ..['CFLAGS'] =
              '-O3 -Werror -arch ${target.goarch} -isysroot ${toolchain.sysroot} -miphoneos-version-min=15.0';
      } else {
        env
  ...
  Future<String> _calculateFingerprint(Target target) async {
    ...
    if (target.isLib) {
      ...
      final versionText = '${(compilerVersion.stdout as String).trim()}\n'
          '${(compilerVersion.stderr as String).trim()}';
      if (target.goos == 'android') {
        builder.addValue('android_compiler', versionText);
        ...
        builder.addValue('ios_compiler', versionText);
        builder.addValue('ios_cflags', env['CFLAGS']);
      }
    }
  ...
  Future<List<String>> _adjustIOSOutput({
    required String outDir,
    required String libPath,
  }) async {
    final outputs = <String>[];

    // Collect headers so consumers (Xcode, XCFramework) can find them.
    // The c-archive build emits libclash.h (preamble + exported functions)
    // next to the archive; it includes bride.h and ios/flclash_ios.h, so the
    // include tree must mirror the source layout.
    final includesPath = p.join(outDir, 'includes');
    ensureDir(includesPath);
    _clearDirectory(includesPath);

    final libHeader = p.join(p.dirname(libPath), 'libclash.h');
    final headerSources = <String>[
      if (File(libHeader).existsSync()) libHeader,
      p.join(_corePath, 'bride.h'),
      p.join(_corePath, 'ios', 'flclash_ios.h'),
    ];
    for (final headerPath in headerSources) {
      if (!File(headerPath).existsSync()) continue;
      final relative = p.relative(headerPath, from: _corePath);
      final includePath = p.join(includesPath, relative);
      ensureDir(p.dirname(includePath));
      File(headerPath).copySync(includePath);
      outputs.add(includePath);
    }

    // Package a static XCFramework so the app and the Network Extension can
    // link the same library slice. Skipped when xcodebuild is unavailable.
    final xcodebuildResult = await Process.run('xcodebuild', ['-version']);
    if (xcodebuildResult.exitCode == 0) {
      final xcframework = p.join(outDir, 'FlClash.xcframework');
      if (Directory(xcframework).existsSync()) {
        Directory(xcframework).deleteSync(recursive: true);
      }
      _deleteIfExists(xcframework);
      ...
      if (result.exitCode != 0) {
        ...
        outputs.add(xcframework);
      }
    } else {
      _log.info('xcodebuild not found; skipping XCFramework packaging');
    }
    return outputs;
  }
